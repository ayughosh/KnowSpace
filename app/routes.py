...
def upload_audio():
    ...
    if request.method == "POST":
        ...
        if not text_to_summarize:
            flash(
                "Unsupported file type or no text to summarize. Please provide a PDF, Word, or text file.",
                "warning",
            )
            return redirect(request.url)
        ...
        try:
            ...
            entry = Entry(
                entry_type=detected_type, content=text_to_summarize, summary=summary
            )
        ...
        return render_template(
            "summary.html", transcript=text_to_summarize, summary=summary
        )
...
def history():
    ...
    entries = (
        Entry.query.filter(Entry.user_id == current_user.id)
